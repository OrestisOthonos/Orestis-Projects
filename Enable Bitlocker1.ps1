...
$outputFile = "$desktopPath\BitLockerRecoveryKey.txt"

# Get the BitLocker volume information
$bitLockerVolume = Get-BitLockerVolume

# Initialize an array to hold the recovery keys
$recoveryKeys = @()

# Loop through each BitLocker volume and collect recovery keys with IDs
foreach ($volume in $bitLockerVolume) {
    foreach ($protector in $volume.KeyProtector) {
        if ($protector.RecoveryPassword) {
            $recoveryKeys += "Drive: $($volume.MountPoint) - Recovery Password: $($protector.RecoveryPassword) - ID: $($protector.KeyProtectorId)"
        }
    }
}

# Save the recovery keys to the output file
if ($recoveryKeys.Count -gt 0) {
    $recoveryKeys | Out-File -FilePath $outputFile -Encoding UTF8
    Write-Host "Recovery keys and IDs saved to $outputFile"
} else {
    Write-Host "No BitLocker recovery keys or IDs found."
}
Unregister-ScheduledTask -TaskName "Name Change" -Confirm:$false
Unregister-ScheduledTask -TaskName "BitlockerEnable" -Confirm:$false
Unregister-ScheduledTask -TaskName "BitDefenderInstall" -Confirm:$false
New-ItemProperty -Path 'HKLM:\Software\Microsoft\Windows\CurrentVersion\RunOnce' -Name 'CleanupFiles' -Value "C:\Users\Admin\Downloads\CleanupFiles.bat" -PropertyType String -Force

# SIG # Begin signature block
# MIIe6QYJKoZIhvcNAQcCoIIe2jCCHtYCAQExDzANBglghkgBZQMEAgEFADB5Bgor
# BgEEAYI3AgEEoGswaTA0BgorBgEEAYI3AgEeMCYCAwEAAAQQH8w7YFlLCE63JNLG
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAAH9tJFFW0o2Vo
# 8OKfnkEDbSQpbrXZqW23rdGZrXdH0qCCA2AwggNcMIICRKADAgECAhBg0d5ZM/rs
...
# 0/bkhGjQimf9oJ3pVYm40A0hmV6E+myVnlTkj10lMA0GCSqGSIb3DQEBAQUABIIB
# ACvNFebdh5xf76oxINNPfWgSN730aVidGHbsHqTcbJ2jdrCutGK7wDFlr+ycb+Rv
# RWjisAyerZRUoKEYGPMegXNx68Yzw8mrm7qqMLGFRL+bL5lQ9gxZpA4Lgry7GJ5K
# u2X9smF4eWJj7UKNlgBMRke1L42zHPXSfe/xU0GtHy+Vb6smt/tW0HbgRMD0HB11
# g40gRQKgNZviuX8HAU2z199U7IJTt3jcmfMSmfVNphBGZkhU2iPhsPrxLMbhzDaQ
# 8CpttCtiIP1Un84M31jscrpE9mD7ZRklSk5lTjYzaliQbdEh4z1sd17dzujFTAEq
# FlIDYmR69PGAHQ8g+SH5hgKhghjYMIIY1AYKKwYBBAGCNwMDATGCGMQwghjABgkq
# hkiG9w0BBwKgghixMIIYrQIBAzEPMA0GCWCGSAFlAwQCAgUAMIH4BgsqhkiG9w0B
# CRABBKCB6ASB5TCB4gIBAQYKKwYBBAGyMQIBATAxMA0GCWCGSAFlAwQCAQUABCCd
# fWpr19HYek6HJbwbJ0n6RRPQ8AVfVnkLJgQsEWkY4wIVAJpGrjJzqDvXIjLo1WDO
# T2I/dGCbGA8yMDI1MTIyMzE2MjcwMVqgdqR0MHIxCzAJBgNVBAYTAkdCMRcwFQYD
...
# CSqGSIb3DQEJBTEPFw0yNTEyMjMxNjI3MDFaMD8GCSqGSIb3DQEJBDEyBDBMpOBT
# PcMZ8Z+0yHfKDNKn2/1i1q3fcmHekW01UnxSYiqGzwAEEqR7Tvvg4b7YjUwwggF6
...
# SIb3DQEBAQUABIICACPCMczB8l7DscxcWEMMDyvcZy9iuowIqtRKvlOIFcS2UVkm
# dcVx2bqkHQ+xDmernO9+ZI81qpohhR3uHVZurkn/BLxfxYC9qtLDmEERvkwTgPZO
# G4jiJhHEVH5I8Ta3FliPI7kXxHMl1VxgAhP9u/m9Apr5An6P7SoNaKa+XNcCM2+v
# 9cxX07vacpTwyYl5BnssWQ+7+FD9xVxbMpn2A8n8/Ci5sUyY/1U+cyNVCISt+9Ke
# dL6dnIBbj3M8uIHtIwGtnBuCXK3kkzc4nx8b3sRdMEDBzBNIQdrpO2YylMYW/lJK
# LQzI9AxsTN8CIntarJohQpTvTP15khkMY7K3fjQuuAmne6LWHLyKuR7CBGn4TONX
# O5f2dcrD4G9zu7Y6Y9VppoEo4C2LUmYRwzepJLOB+Pu+Kg1J+S245H3Q4202rqM5
# 75kaDaQW2ZQY51LgnB2kbztJJ4ag9ZZmBs4czqgF6Xeu9AsxwAol5mg1pduJDXnS
# EmKKUBSvQiZh38tO3koN68sKc+Culcz7Z5jrtj6luu97atpi9ahVCfTaLhmw83T3
# eN1Nk81d2tuXse32PLM2v2d7ysmSalXJzuX04e6ygxxY7AIHYZLflLdH7DC+pVjV
# XW13MmCaxz6RoOHgc5GYoYV5h9HkfilbgSx6xIQjYvvDk0AparvIGtiUIgsW
